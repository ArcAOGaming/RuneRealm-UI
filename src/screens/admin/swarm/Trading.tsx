...
import { OPEN_ORDER_CAP, clockTime, fmtInt, fmtPx, fmtRate, marketRows, openOrders } from './model';
  ...
  return (
    <Panel className="min-w-0 p-4 sm:p-5" data-card="trading">
      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="text-sm font-medium text-ink">Markets</h2>
        <span className="text-[11px] text-faint">every fill is fleet against fleet, by construction</span>
      </div>
      <div className="mb-3 grid grid-cols-2 gap-px overflow-hidden rounded-[3px] border border-edge/60 bg-edge/60 sm:grid-cols-3">
        <div className="bg-surface/95 p-3">
          <div className="text-xs text-muted">Fleet open orders</div>
          <div className="mt-1 font-mono text-xl text-ink">{fmtInt(open)}</div>
          <div className="mt-0.5 truncate text-[10px] text-faint">both venues · cap {fmtInt(OPEN_ORDER_CAP)} per venue</div>
        </div>
        <div className="bg-surface/95 p-3">
          <div className="text-xs text-muted">Maker share</div>
          <div className="mt-1 font-mono text-xl text-ink">{share === null ? '—' : `${share}%`}</div>
        </div>
        <div className="bg-surface/95 p-3">
          <div className="text-xs text-muted">Orders held</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full whitespace-nowrap text-left font-mono text-[11px]">
            <thead><tr className="text-[10px] uppercase tracking-wider text-faint">
              <th className="py-1 pr-3">Venue</th><th className="pr-3">Market</th><th className="pr-3 text-right" title="whole units of the quote asset">Last px</th>
            ...
            <tbody>{rows.map((row) => (
              <tr key={row.key} className="border-t border-edge/30">
                ...
                <td className="pr-3 text-right text-ink" title={row.lastAt ? clockTime(row.lastAt) : undefined}>{fmtPx(row.lastPx)}</td>
